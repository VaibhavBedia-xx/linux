...
static irqreturn_t wkup_m3_txev_handler(int irq, void *unused)
{
	...
	case M3_STATE_RESET:
		...
		wkup_m3->version = omap_ctrl_readl(AM33XX_CONTROL_IPC_MSG_REG2) & 0x0000ffff;
		if (wkup_m3->version == M3_VERSION_UNKNOWN ||
			wkup_m3->version < M3_BASELINE_VERSION) {
...
int __init am33xx_pm_init(void)
{
	...
	if (ret) {
		...
		suspend_cfg_param_list[MEMORY_TYPE] = temp;
